...
1. PermissionsGuard crashed the API on startup

- Used @InjectRepository(Role/UserRole) but was registered as APP_GUARD (global context) and via  
  @UseGuards() in modules that never called TypeOrmModule.forFeature([Role, UserRole])
- NestJS couldn't resolve the repositories → startup crash → API never listened on port 4000
- Fix: Replaced @InjectRepository with @InjectDataSource — DataSource is globally available without  
  forFeature()

2. AllExceptionsFilter silenced the real error

- In development, it did errorResponse.rawError = exception then called res.json(errorResponse)
- NestJS/TypeORM exception objects have circular references → JSON.stringify threw → Express caught  
  that crash and returned raw plain-text "Internal Server Error"
- So the 500 was unreadable, hiding root cause #1
- Fix: Serialize exception to { message, stack } string instead of raw object

3. JWT secret mismatch broke the redirect

- API signed tokens with super_secret_jwt_key_change_me_12345 (from apps/api/.env)
- Next.js middleware verified tokens with 683abb93... (from apps/web/.env.local)
- Verification always failed → middleware redirected back to /login after every successful login
- Fix: Synced apps/web/.env.local to use the same secret as the API
