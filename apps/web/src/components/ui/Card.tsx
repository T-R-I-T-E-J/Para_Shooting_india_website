...
export const NewsCard = ({ title, excerpt, category, date, href, imageUrl, imageClassName }: NewsCardProps) => {
  return (
    <Link 
      href={href} 
      ...
      <div className="relative h-52 w-full shrink-0 overflow-hidden bg-neutral-100">
        {imageUrl ? (
          <img 
            src={imageUrl} 
            alt={title} 
            className={clsx("h-full w-full object-cover transition-transform duration-500 group-hover:scale-105", imageClassName)}
          />
        ) : (
