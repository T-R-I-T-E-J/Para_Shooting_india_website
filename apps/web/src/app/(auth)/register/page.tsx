...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        let errorMsg = responseData.message || 'Registration failed'
        if (Array.isArray(errorMsg)) {
          errorMsg = errorMsg[0] // just show the first validation error
        }
        throw new Error(errorMsg)
      }

      // Handle successful registration explicitly setting token as cookie
      const { user, access_token } = responseData.data || responseData
      if (access_token) {
        const maxAge = 7 * 24 * 60 * 60;
        document.cookie = `auth_token=${access_token}; path=/; max-age=${maxAge}; samesite=lax; secure`;
      }
