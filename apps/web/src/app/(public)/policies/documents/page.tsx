...
const CATEGORY_META: Record<string, { label: string; color: string }> = {
  rules:                  { label: 'Rules',          color: '#C8A415' },
  selection:              { label: 'Selection',       color: '#003DA5' },
  calendar:               { label: 'Calendar',        color: '#046A38' },
  ...
  match:                  { label: 'Match',           color: '#475569' },
}
...
async function getPolicyDocs(): Promise<PolicyDoc[]> {
  try {
    const apiUrl =
...
export const metadata = {
  title: 'Policy Library — Para Shooting India',
  description:
    'Browse and download official rules, selection criteria, calendars, and governance documents for Para Shooting India.',
}
...
export default async function PolicyDocumentsPage({
  ...
  const allDocs = await getPolicyDocs()
  ...
  function buildUrl(overrides: Record<string, string>) {
    ...
    return `/policies/documents${str ? `?${str}` : ''}`
  }
    ...
    <div className="bg-white min-h-screen">
      ...
      <section className="relative pt-32 pb-16 px-6 overflow-hidden bg-primary">
        ...
        <div className="max-w-7xl mx-auto relative">
          <nav className="flex items-center gap-2 text-[11px] font-bold tracking-[0.15em] uppercase text-white/40 mb-6">
            <Link href="/policies" className="hover:text-[#C8A415] transition-colors">
              Policies
            </Link>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <h1 className="font-heading text-5xl md:text-6xl font-bold text-white leading-[1.05]">
              Document<br />
              <em className="text-[#C8A415] not-italic">Library</em>
            </h1>
            <div className="flex items-center gap-3 pb-2">
              <div className="w-1.5 h-1.5 rounded-full bg-[#C8A415] animate-pulse" />
              <span className="text-white/50 text-[13px]">
                <span className="text-white font-semibold">{totalAll}</span> documents total
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm border-b border-neutral-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col sm:flex-row gap-4">
          ...
          {/* Search */}
          <form method="GET" action="/policies/documents" className="relative flex-1 max-w-md">
            ...
            <p className="text-neutral-500 text-[12px]">
              {sq || activeCat ? (
                <>
                  ...
                  {activeCat && (
                    <> in <span className="text-neutral-700">{CATEGORY_META[activeCat]?.label ?? activeCat}</span></>
                  )}
                </>
              ) : (
                <>
            ...
            <div className="py-24 flex flex-col items-center justify-center text-center border border-neutral-200 bg-neutral-50">
              ...
              {sq ? (
                <p className="text-neutral-400 text-sm mb-6">
                  ...
                  {activeCat && <> in {CATEGORY_META[activeCat]?.label ?? activeCat}</>}.
                </p>
              ) : (
                <p className="text-neutral-400 text-sm mb-6">
                  No {activeCat ? CATEGORY_META[activeCat]?.label : ''} documents are available yet.
                </p>
              )}
              <Link
                href="/policies/documents"
      ...
      <section className="py-16 px-6 border-t border-neutral-200 bg-neutral-50">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <h2 className="font-heading text-3xl font-bold text-neutral-900 mb-4">Looking for something else?</h2>
          <p className="text-neutral-500 text-sm mb-8">
            If you cannot find the governance document you are looking for, contact the STC office for assistance.
          </p>
