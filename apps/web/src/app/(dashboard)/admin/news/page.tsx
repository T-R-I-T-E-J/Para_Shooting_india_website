...
const AdminNewsPage = () => {
  ...
  const fetchNews = async () => {
    try {
      const res = await fetch('/api/v1/news', {
        // credentials: 'same-origin' is default for relative paths
      })
      if (res.ok) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      // Use relative path (/api/v1) to ensure request goes through Next.js proxy and sends cookies
      const res = await fetch(`/api/v1/news/${itemToDelete}`, {
        method: 'DELETE',
        credentials: 'include',
        headers: {
